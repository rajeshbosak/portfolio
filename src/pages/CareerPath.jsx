...
export default function CareerPath() {
  return (
    <div id="career">
      <h3 className="text-2xl font-semibold">Career Path</h3>
      ...
      <div className="mt-6 space-y-6">
        {events.map((e, i) => (
          <div key={i} className="flex gap-6 items-start">
            <div className="w-20 text-indigo-300 font-semibold">{e.year}</div>
            <div className="card-glass flex-1">
              <div className="font-semibold">{e.title}</div>
              <div className="text-sm text-slate-400 mt-2">{e.desc}</div>
            </div>
          </div>
        ))}
      </div>
